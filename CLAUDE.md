...
**Content flow:** every post exists in BOTH languages as two same-named Markdown files: `src/content/blog/en/<slug>.md` → `/blog/<slug>/` and `src/content/blog/vi/<slug>.md` → `/vi/blog/<slug>/`. When asked to write or add a post, always create both files (translate, don't just copy). Frontmatter (validated by zod in `src/content.config.ts`): `title`, `description`, `pubDate`, `draft`, `tags` — language comes from the folder, never from frontmatter. Tags are lowercase language-neutral slugs (e.g. `astro`, `webdev`) and must be identical in both language files of a pair; the homepage renders them as filter tabs (client-side script in `Home.astro` — the only handwritten JS on the site besides the giscus embed). All post access goes through `src/lib/blog.ts`: `getPosts(lang)` (sorted, drafts excluded) plus `postLang()`/`postSlug()` helpers; it throws at build time if any post is missing one language version, so a half-translated pair fails fast. Routes: `src/pages/blog/[slug].astro` (en) and `src/pages/vi/blog/[slug].astro` (vi), both thin wrappers around `src/components/PostArticle.astro`; the full post list lives at `/blog/` + `/vi/blog/` (wrappers around `src/components/Blog.astro`, which renders `PostList` with tag filter tabs). The homepage shows only the latest 5 posts via the same `PostList` component (no tabs) with a "view all" link. Drafting a post means setting `draft: true` in BOTH files.
